import { IconType } from './tools/tool';
...
export class Toolbox {
  ...
  constructor(p: P5) {
    ...
    this.toolbarItemClick = function () {
      ...
      const toolName = (this as unknown as P5.Element).id().split(SIDEBAR_ITEM_IDENTIFIER)[0];
  ...
  selectTool(toolName: string) {
    for (let i = 0; i < this.tools.length; i++) {
      if (this.tools[i].name == toolName) {
        ...
        this.p.select(`#${toolName}${SIDEBAR_ITEM_IDENTIFIER}`)?.addClass(SIDEBAR_ITEM_ACTIVE_CLASSES);
